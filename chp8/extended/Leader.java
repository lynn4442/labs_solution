...
public class Leader {
    ...
    public void processRequest() {
        ...
        if (ackCount >= MAJORITY) {
            ...
            System.out.println("[Leader] Log: " + log);
        } else {
            ...
            for (int i : ackedFollowers) {
                ...
                for (int attempt = 1; attempt <= 3 && !sent; attempt++) {
                    try {
                        ...
                        System.out.println("Follower " + (i + 1) + " rollback attempt " + attempt + " failed");
                        try { Thread.sleep(500); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                    }
                }
                if (!sent)
                    System.out.println("WARNING: follower " + (i + 1) + " could not be rolled back — log inconsistent");
            }
